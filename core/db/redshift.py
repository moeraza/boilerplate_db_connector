...
def connect_to_s3(aws_access_key_id=None, aws_secret_access_key=None, bucket=None, subdirectory=None, aws_iam_role=None, **kwargs):
...
def df_to_s3(data_frame, csv_name, index, save_local, delimiter, verbose=True, **kwargs):
    ...
    file_key = s3_subdirectory_var + csv_name
    return file_key
...
def s3_to_redshift(redshift_table_name, csv_name, delimiter=',', quotechar='"',
    ...
    authorization = "iam_role '{0}'".format(REDSHIFT_IAM_ROLE)
...
def pandas_to_redshift(data_frame,
    ...
    s3_kwargs = {k: v for k, v in kwargs.items()
        if k in S3_ACCEPTED_KWARGS and v is not None}
    file_key = df_to_s3(data_frame, csv_name, index, save_local, delimiter, verbose=verbose, **s3_kwargs)
...
def query_redshift(query_str, limit=100):
    if limit:
        query_str = query_str + ' limit {0}'.format(limit)
    ...
    df = redshift_to_pandas(query_str)
    close_up_shop()
    return df

def execute_query_redshift(query_str):
    connect_to_redshift(dbname = REDSHIFT_DBNAME,
                    host = REDSHIFT_HOST,
                    port = REDSHIFT_PORT,
                    user = REDSHIFT_USER,
...
def pushtors(df, tbl_name, distkey, sortkey, append = True):
    ...
    connect_to_s3(bucket = S3BUCKETNAME,subdirectory=S3SUBDIRECTORYNAME)
    pandas_to_redshift( data_frame = df,
                        redshift_table_name = tbl_name, 
                        append = append,
                        distkey=distkey, sortkey=sortkey
                        )
    exec_commit('GRANT SELECT ON {0} TO PUBLIC'.format(tbl_name))
    close_up_shop()